...
3. Orphan link blocks. A paragraph that's just a naked "<a>Title of some article</a>" dropped mid-section with no surrounding prose reads like a glitch. Weave it into a natural sentence INSIDE THE SAME PARAGRAPH BLOCK. Do NOT "fix" it by converting it into a bullet list, by adding a heading above it, by adding a marketing intro sentence, or by inventing any new block structure around it. The only allowed fix is to rewrite the paragraph's sentence so the link reads naturally in prose. If no natural in-prose fix works, leave it alone and mention it in the change summary.
   ...
   IMPORTANT EXCEPTION: list items sitting inside a pre-existing list under a heading like "Related Reading", "Related Articles", "More from us", "See also", or similar (or under a bold-only label paragraph that acts as such a heading) are deliberate link-only list items by design. Do NOT rewrite them as prose. Do NOT modify the anchor text inside these bullets, even if a locked anchor for the same href has different wording. Bullets under a Related Reading-style heading are NOT contextual prose placements; the bullet text is the editorial choice and must be preserved byte-for-byte. If a locked link's href already lives inside such a list, treat the lock as satisfied and leave the bullet exactly as written, regardless of whether its anchor text matches the locked anchor string.
...
a. Every locked anchor text for a segment MUST appear verbatim in that segment's output, inside <a href="…"> tags with the exact href. EXCEPTION: if the locked link's href already appears inside a Related Reading-style list (a list under a heading or bold-only label paragraph titled "Related Reading", "Related Articles", "More from us", "See also", or similar), the existing list-item is the canonical placement. Leave the bullet's anchor text unchanged; do not rewrite it to match the locked string. The lock is considered satisfied by href presence alone in that case.
